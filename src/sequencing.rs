...
pub fn chords<const N: usize>(notes: [ChordValue; N]) -> SequenceValue {
    match N {
        2 => SequenceValue::Half([Some(notes[0]), Some(notes[1])]),
        3 => SequenceValue::Third([Some(notes[0]), Some(notes[1]), Some(notes[2])]),
...
impl Sequence {
    pub fn elements(&mut self, elements: SequenceValue) -> &mut Self {
